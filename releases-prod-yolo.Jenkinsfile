pipeline {
    ...
    parameters {
        string(name: 'JENKINS_YOLO_PROD_IMG_URL', defaultValue: '', description: '')
    }
        ...
        stage('Deploy') {
            steps {
              ...
              withCredentials([string(credentialsId: 'jenkinsclassicgithubtoken', variable: 'jenkinsclassicgithubtoken')]) {
                sh 'git config --global --add safe.directory /var/lib/jenkins/workspace/prod/releases-prod-yolo'
                ...
                sh 'git branch'
                sh 'cd k8s/prod && ls'
                sh "sed -i \"s|image: .*|image: $JENKINS_YOLO_PROD_IMG_URL|\" k8s/prod/yolo.yaml"
                sh 'git add k8s/prod/yolo.yaml'
                sh 'git commit -m "$JENKINS_YOLO_PROD_IMG_URL" '
